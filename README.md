...
以 [p708 工程](https://github.com/YunaiV/ruoyi-vue-pro) 为最新标准：
...
**注意**：`lingman-core/` 不是 Skill，是共享知识层，放在根目录下即可，**不要**把它放进 `.claude/skills/`。
...
| 配置项 | p705（旧） | p708（新/默认） |
|--------|-----------|----------------|
| DO 基类 | `TenantBaseDO` | `BaseDO` |
| ID 策略 | `IdType.ASSIGN_ID`（雪花算法） | `@KeySequence` + `IdType.INPUT`（PG 序列） |
| 转换器 | `BeanUtils.copyProperties` | MapStruct `@Mapper` |
| 注入方式 | `@Autowired` | `@Resource` |

Skills 默认以 **p708** 规范为准。如果项目使用 p705 规范，生成后手动调整即可。
